 ...
 * 2. Create new Gemini tab via MQTT
...
const MQTT_TOPIC = "claude/browser/command"
...
await $`mosquitto_pub -h ${MQTT_HOST} -p ${MQTT_PORT} -t ${MQTT_TOPIC} -r -m ${createTabCmd}`
...
await $`mosquitto_pub -h ${MQTT_HOST} -p ${MQTT_PORT} -t ${MQTT_TOPIC} -r -m ${chatCmd}`
